 ...
 * the tapped icon (iOS-style) and back out on close. Checkpoint 3
 * ships a placeholder per app; the real app screens replace it in
 * later stages.
 *
 * The zoom uses React Native's built-in Animated (native-driver scale
 * + fade) — chosen for zero setup risk on this triggered transition.
 * Reanimated remains the stack choice for the later gesture-driven
 * animations (e.g. the swipe minigame).
 */
...
export function AppView({ origin }: AppViewProps) {
  ...
  useEffect(() => {
    ...
    // `displayed` is intentionally not a dependency — see the comment
    // above; it is read only to gate the close animation.
  ...
  return (
    ...
    >
      ...
      <View style={[styles.body, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.note}>This screen is built in a later stage</Text>
      </View>
