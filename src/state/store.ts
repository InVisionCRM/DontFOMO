 ...
 * §5.
 *
 * Checkpoint 2 added the clock and core stats. Checkpoint 3 adds the
 * persistent-save slice and the load action. Later stages add the
 * market, the wallet, scams, and so on.
 */
 ...
 * The clock, cash, followers and handle survive a quit; transient UI
 * state (such as which app is open) is intentionally not saved.
 */
...
export const useGameStore = create<GameState>()((set) => ({
  ...
  loadSaved: (saved, now) =>
    set({
      clock: resumeClock(saved.clock, now).clock,
      cash: saved.cash,
